(ns mailchimp.main
  "Kotodama WASM entrypoint for the Mailchimp clean-room actor (L4) — Clojure port.
  ...
  facts (`mailchimp.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mailchimp")
(def tier "L4")
...
(def entity-specs
  [{:entity "List"       :plural "lists"       :id-prefix "mailchim_lis"
    :fields [:name :permissionReminder :useArchiveBar :notifyOnSubscribe :notifyOnUnsubscribe :listRating] :required [:name :permissionReminder]
    :coerce {:useArchiveBar :bool :notifyOnSubscribe :bool :notifyOnUnsubscribe :bool :listRating :int} :refs {}}
   {:entity "ListMember" :plural "listmembers" :id-prefix "mailchim_lis"
    :fields [:emailAddress :emailType :status :language :vip :memberRating] :required [:emailAddress :emailType]
    :coerce {:vip :bool :memberRating :int}  :refs {}}
   {:entity "Campaign"   :plural "campaigns"   :id-prefix "mailchim_cam"
    :fields [:type :contentType :status :emailTitle] :required [:type :contentType]
    :coerce {}                       :refs {}}
   {:entity "Template"   :plural "templates"   :id-prefix "mailchim_tem"
    :fields [:name :type :folderId] :required [:name :type]
    :coerce {}                       :refs {}}
   {:entity "Segment"    :plural "segments"    :id-prefix "mailchim_seg"
    :fields [:name :type :memberCount] :required [:name :type]
    :coerce {:memberCount :int}      :refs {}}
   {:entity "Automation" :plural "automations" :id-prefix "mailchim_aut"
    :fields [:workflowId :type :status] :required [:type :status]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"mailchimp.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mailchimp-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mailchimp-compat/ready)
